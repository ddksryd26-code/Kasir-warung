...
function chartPoints(period: ReportPeriod, sales: ReturnType<typeof useWarung>['sales'], expenses: ReturnType<typeof useWarung>['expenses']): ChartPoint[] {
  ...
  return dates.map((date) => {
    ...
    const revenue = sales.filter((sale) => sale.date === key && isActiveSale(sale)).reduce((sum, sale) => sum + sale.amount, 0);
...
export default function ReportsScreen() {
  ...
  const metrics = calculateReportMetrics(periodSales, menus, inventory, consignments, periodExpenses);
  const { revenue, ingredientCost, consignmentCost, grossProfit, expenses: costs, netProfit } = metrics;
  ...
  const remainingAfterSavings = netProfit - savingsTotal;
  ...
  return (
    <Screen>
      ...
      <Surface tone="ink" style={s.profit}><Text style={[s.profitLabel, { color: c.mutedForeground }]}>LABA SETELAH PENGELUARAN · {period.toUpperCase()}</Text><Text style={[s.profitValue, { color: c.card }]}>{formatRp(netProfit)}</Text><Text style={[s.profitSub, { color: c.mutedForeground }]}>{formatRp(revenue)} penjualan  −  {formatRp(metrics.totalCostOfGoods)} HPP  −  {formatRp(costs)} pengeluaran</Text></Surface>
      <SectionHeader title="HPP & laba kotor" icon="calculator-outline" />
      <View style={s.grid}><Surface style={s.metric}><Ionicons name="restaurant-outline" size={20} color={c.primary} /><Text style={[s.metricLabel, { color: c.mutedForeground }]}>HPP bahan menu</Text><Text style={[s.metricValue, { color: c.foreground }]}>{formatRp(ingredientCost)}</Text></Surface><Surface style={s.metric}><Ionicons name="storefront-outline" size={20} color={c.primary} /><Text style={[s.metricLabel, { color: c.mutedForeground }]}>HPP barang titipan</Text><Text style={[s.metricValue, { color: c.foreground }]}>{formatRp(consignmentCost)}</Text></Surface></View>
      <Surface style={s.profitBreakdown}><View style={s.breakdownRow}><Text style={[s.breakdownLabel, { color: c.mutedForeground }]}>Pendapatan</Text><Text style={[s.breakdownValue, { color: c.foreground }]}>{formatRp(revenue)}</Text></View><View style={s.breakdownRow}><Text style={[s.breakdownLabel, { color: c.mutedForeground }]}>Laba kotor</Text><Text style={[s.breakdownValue, { color: c.primary }]}>{formatRp(grossProfit)}</Text></View><View style={[s.breakdownRow, { borderTopColor: c.border }]}><Text style={[s.breakdownLabel, { color: c.mutedForeground }]}>Setelah pengeluaran</Text><Text style={[s.breakdownValue, { color: netProfit >= 0 ? c.foreground : c.destructive }]}>{formatRp(netProfit)}</Text></View></Surface>
      {(metrics.missingIngredientIds.length || metrics.missingConsignmentIds.length) ? <Text style={[s.costHint, { color: c.mutedForeground }]}>Beberapa HPP belum dihitung karena harga beli bahan atau data penitip belum diisi.</Text> : null}
...
const s = StyleSheet.create({
  ...
  profitBreakdown: { marginBottom: 8 },
  ...
  costHint: { fontSize: 11, lineHeight: 16, marginBottom: 22 },
});