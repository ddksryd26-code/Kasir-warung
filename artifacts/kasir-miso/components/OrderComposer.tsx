...
export function OrderComposer({ targetOrder = null, onComplete, onCancel }: OrderComposerProps) {
  ...
  const [variantPicker, setVariantPicker] = useState<CatalogItem | null>(null);
  ...
  const getItemPrice = (item: OrderItem) => {
    const catalogItem = catalogItems.find((catalogItem) => catalogItem.id === item.menu);
    const variant = catalogItem?.variants?.find((entry) => entry.id === item.variantId);
  ...
  const changeQty = (key: MenuKey, delta: number, variantId?: string) => {
    haptic();
    const found = cart.find((item) => item.menu === key && item.variantId === variantId);
    if (delta > 0 && (found?.qty ?? 0) >= getMenuAvailability(key, variantId)) return;
    setCart((current) => {
      if (!found && delta > 0) return [...current, { menu: key, qty: 1, variantId }];
      return current
        .map((item) => item.menu === key && item.variantId === variantId ? { ...item, qty: Math.max(0, item.qty + delta) } : item)
        .filter((item) => item.qty > 0);
    });
  };
  const addMenu = (item: CatalogItem) => {
  ...
  const removeMenu = (item: CatalogItem) => {
    const variantId = [...cart].reverse().find((entry) => entry.menu === item.id)?.variantId;
    changeQty(item.id, -1, variantId);
  };
  ...
  const submit = () => {
    ...
    if (cart.some((item) => item.qty > getMenuAvailability(item.menu, item.variantId))) {
  ...
  return (
    <>
      <Surface style={s.composer}>
        ...
        <View style={s.menuGrid}>
          {visibleMenus.map((item) => {
            const qty = cart.filter((entry) => entry.menu === item.id).reduce((sum, entry) => sum + entry.qty, 0);
      ...
      <Modal visible={!!variantPicker} transparent animationType="slide" onRequestClose={() => setVariantPicker(null)}>
        <View style={[s.modalBackdrop, { backgroundColor: c.foreground + 'B8' }]}>
          <View style={[s.variantModal, { backgroundColor: c.card }]}>
            <View style={s.modalTitleRow}>
              <View><Text style={[s.modalKicker, { color: c.primary }]}>PILIHAN MENU</Text><Text style={[s.modalTitle, { color: c.foreground }]}>{variantPicker?.name}</Text></View>
              <Pressable accessibilityLabel="Tutup pilihan varian" onPress={() => setVariantPicker(null)}><Ionicons name="close-circle" size={27} color={c.mutedForeground} /></Pressable>
            </View>
            <Text style={[s.muted, { color: c.mutedForeground }]}>Pilih varian sebelum menambahkan menu ke pesanan.</Text>
            <View style={s.variantList}>
              {variantPicker?.variants?.map((variant) => {
                const price = (variantPicker.price ?? 0) + variant.priceDelta;
                const unavailable = getMenuAvailability(variantPicker.id, variant.id) <= (cart.find((entry) => entry.menu === variantPicker.id && entry.variantId === variant.id)?.qty ?? 0);
                return <Pressable key={variant.id} disabled={unavailable} onPress={() => { changeQty(variantPicker.id, 1, variant.id); setVariantPicker(null); }} style={({ pressed }) => [s.variantOption, { borderColor: unavailable ? c.muted : c.border, backgroundColor: unavailable ? c.background : c.secondary, opacity: pressed ? 0.7 : 1 }]}>
                  <View style={s.flex}><Text style={[s.variantName, { color: unavailable ? c.mutedForeground : c.foreground }]}>{variant.name}</Text><Text style={[s.variantSub, { color: c.mutedForeground }]}>{unavailable ? 'Stok habis' : 'Tambahkan ke pesanan'}</Text></View>
                  <Text style={[s.variantPrice, { color: unavailable ? c.mutedForeground : c.primary }]}>{formatRp(price)}</Text>
                </Pressable>;
              })}
            </View>
          </View>
...
const s = StyleSheet.create({
  ...
  variantHint: { fontSize: 9, marginTop: 2 },
  ...
  variantModal: { borderTopLeftRadius: 27, borderTopRightRadius: 27, padding: 21, paddingBottom: 31 },
  variantList: { gap: 8, marginTop: 16 },
  variantOption: { minHeight: 58, borderWidth: 1, borderRadius: 14, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 10 },
  variantName: { fontSize: 13, fontWeight: '800' },
  variantSub: { fontSize: 10, marginTop: 3 },
  variantPrice: { fontSize: 12, fontWeight: '800' },
