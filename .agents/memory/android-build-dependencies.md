...
**How to apply:** Keep `react`, `react-dom`, `react-native`, and `react-test-renderer` aligned; use an exact renderer version when React is exact, update the workspace lockfile, keep `minimumReleaseAge` enabled, and simulate a clean `pnpm install --frozen-lockfile` from the workspace. Use `allowBuilds: {package-name: true}` for required lifecycle scripts and `false` for scripts that should remain blocked.